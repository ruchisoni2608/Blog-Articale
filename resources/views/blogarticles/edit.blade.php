...
@section('content')
    <div class="container mt-2">
        ...
        <form action="{{ route('blogarticles.update', $blogarticle->id) }}" method="POST" enctype="multipart/form-data"
            id="singleFileUpload">
            <!-- @csrf -->
            {!! csrf_field() !!}
            @method('PUT')
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        <label> Title:</label>
                        <input type="text" name="title" class="form-control" value="{{ $blogarticle->title }}">
                    </div>
                </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        <textarea id="editor1" name="content" cols="30" rows="10">{{ strip_tags($blogarticle->content) }}</textarea>
                    </div>
                </div>
                        ...
                        <div class="custom-select">
                            <select class="form-select" id="exampleFormControlSelect9" name="category_id">
                                ...
                                @foreach ($blogcategories as $category)
                                    <option @selected($category->id == $blogarticle->category_id) value="{{ $category->id }}">{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
        ...
        if (typeof Dropzone !== "undefined" && !$("#singleFileUpload").hasClass("dz-clickable")) {
            $(document).ready(function() {
                ...
                var myDropzone = new Dropzone("#singleFileUpload", {
                    url: "{{ route('blogarticles.update', $blogarticle->id) }}",
                    paramName: "image",
                    maxFilesize: 2,
                    ...
                    init: function() {
                        ...
                        var existingImages =
                            "{{ $blogarticle->image }}"; // Get the existing image filename as a string
                        ...
                        if (existingImages) {
                            ...
                            this.emit("thumbnail", mockFile, "{{ asset('images') }}/" +
                                existingImages);
                        ...
                        this.on('success', function(file, response) {
                            ...
                            window.location.href = "{{ route('blogarticles.index') }}";
                        });
